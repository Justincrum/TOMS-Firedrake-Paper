...
Additionally, there are small adjustments made to Firedrake's UFL and TSFC definitions so that each of the packages know to look for the trimmed serendipity elements.  The changes to TSFC and UFL are what then allow the user to make a call to the trimmed serendipity elements in a Firedrake code (see Listing~\ref{lst:pde_using_trimmed_serendipity}).  The FInAT adjustments are done by adding in the relevant element names and properties to {\fontfamily{qcr}\selectfont fiat\_elements.py} and {\fontfamily{qcr}\selectfont \_\_init\_\_.py}. To adjust TSFC to recognize the trimmed serendipity elements, we adjust {\fontfamily{qcr}\selectfont fiatinterface.py} and {\fontfamily{qcr}\selectfont finatinterface.py} so that they look for the {\fontfamily{qcr}\selectfont SminusCurl} and {\fontfamily{qcr}\selectfont SminusDiv} aliases.  Finally, we edit {\fontfamily{qcr}\selectfont elementlist.py}, adding in {\fontfamily{qcr}\selectfont register\_element2} functions for the trimmed serendipity elements. % \josh{I think it would be appropriate to include a small table at this point, that both shows what the function spaces are called as in Firedrake vs. the mathematics expression (e.g. SminusCurl, SminusDiv, etc.) AND shows which function spaces they are designed to work with (e.g. DPC).  This will hopefully give more insight on how to use our implementation}

\begin{table}
\begin{tabular}{|l|l|l|l|}
\cline{1-4}
& FEEC Notation & Firedrake (2D) & Firedrake (3D) \\\cline{1-4}
 Tensor Product $H^1$ & $\calQ^-_r \Lambda^0$ & {\fontfamily{qcr}\selectfont Lagrange} & {\fontfamily{qcr}\selectfont Lagrange}  \\\cline{1-4}
  Tensor Product \hcurl& $\calQ^-_r \Lambda^1$ & {\fontfamily{qcr}\selectfont RTCE or RTCF} & {\fontfamily{qcr}\selectfont NCE} \\\cline{1-4}
 Tensor Product \hdiv& $\calQ^-_r \Lambda^2$ & {\fontfamily{qcr}\selectfont DQ} & {\fontfamily{qcr}\selectfont NCF}  \\\cline{1-4}
  Tensor Product $L^2$& $\calQ^-_r \Lambda^3$ & {\fontfamily{qcr}\selectfont -} & {\fontfamily{qcr}\selectfont DQ}\\ \cline{1-4}
 Trimmed Serendipity $H^1$ & $\calS^-_r \Lambda^0$  & {\fontfamily{qcr}\selectfont S} & {\fontfamily{qcr}\selectfont S}  \\\cline{1-4}
 Trimmed Serendipity \hcurl& $\calS^-_r \Lambda^1$ & {\fontfamily{qcr}\selectfont SminusCurl or SminusDiv} & {\fontfamily{qcr}\selectfont SminusCurl}  \\\cline{1-4}
 Trimmed Serendipity \hdiv& $\calS^-_r \Lambda^2$ & {\fontfamily{qcr}\selectfont DPC} & {\fontfamily{qcr}\selectfont SminusDiv}\\ \cline{1-4}
 Trimmed Serendipity $L^2$& $\calS^-_r \Lambda^3$ & {\fontfamily{qcr}\selectfont -} & {\fontfamily{qcr}\selectfont DPC}\\ \cline{1-4}
\end{tabular}
\caption{A translation for going from Finite Element Exterior Calculus to Firedrake usage names for tensor product and trimmed serendipity elements.\akg{I like this table now. Can we pad the vertical spacing a little so it's easier to read?}}
...
\begin{figure}[h!]
  ...
  \begin{subfigure}[h]{0.5\textwidth}
    ...
    \caption{3D primal Poisson error analysis.}
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[h]{0.5\textwidth}
    \centering
        \caption{3D Primal DOFs vs Time}
          \begin{tikzpicture}[scale=0.88]
