...
\red{Tables for eigenvalue comparison for Maxwell resonator problem go in this section}
...
When testing out H$($curl$)$ elements, we choose to use the Maxwell Eigenvalue problem.  The results are summarized in the following table:
...
\hline
Compute Time & 3:25:09 & 3:40:53 & 7:29:12 & 9:58:35 \\
\hline
Wallclock Time & 2:37 & 2:48 & 8:07 & 6:49 \\
\hline
...
\hline
Compute Time & 1:25:09 & 1:31:02 & 1:19:39 & 2:46:42 \\
\hline
Wallclock Time & 0:01:40 & 0:01:20 & 0:01:15 & 0:02:10 \\
\hline

\end{tabular}
\caption{A comparison of how NCE and $S^-$ finite elements solve the Maxwell cavity resonator eigenvalue problem, $\langle \text{curl}(F), \text{curl}(E) \rangle = \omega^2 \langle F, E \rangle$. }  
\end{table}
\label{tab:Eigenvalue}
\end{center}

In \ref{tab:Eigenvalue}, we look at the convergence rates of difference eigenvalues based off meshes built with tensor product (NCE) elements and trimmed Serendipity ($S^-$) elements in 3-d.  We use H$($curl$)$ to solve the problems, corresponding with edge elements in 3-d.  Based off earlier eigenvalue works (Boffi, FIND ANOTHER REFERENCE PROBABLY), we expect that the rate of convergence be double the order of the finite element used to solve the problem.  In most of the table, this is exactly what we see.  The experiment was done by using SLEPc in Firedrake, computing an inverted shift to a target of $3.0$, then asking SLEPc for $15$ eigenvalue, eigenvector pairs.  We ignored the eigenvalues of $1$, as they correspond only to the boundary conditions, whereas the actual eigenvalues that we expect to see follow the formula:

\[ \omega^2 = m_1^2 + m_2^2 + m_3^2 \]

where $m_i \in \mathbb{N} \cup {0}$ and no more than one of $m_1, m_2, m_3$ may be equal to $0$ at a time.  Another note is that $4$ should never be an eigenvalue in 3-d, however, we see it consistently found by both the NCE and $S^-$ elements.  Finally, any eigenvalue that has a $-$ spot is to be interpreted as the eigenvalue solver did not find that specific eigenvalue in the number of iterations it required to find the 15 requested eigenvalue-eigenvector pairs.  
