...
\begin{equation*}
\begin{tabular}{rllll}
$\int_\Omega (\sigma \cdot \tau + \nabla\cdot\tau u) \text{ d}x$ & $=~~ \int_\Gamma \tau \cdot n u \text{ d}x$ & $\forall~ \tau\in\Sigma$, \\
$\int_\Omega \nabla \cdot \sigma v \text{ d}x$ & $= ~~-\int_\Omega fv \text{ d}x$ & $\forall~ v\in V$.
\end{tabular}
\end{equation*}

%\begin{equation*}
%\begin{aligned}
%  \int_\Omega (\sigma \cdot \tau + \nabla \cdot \tau u + \nabla \cdot \sigma 5\nu) \text{d}x &= - \int_\Omega f \nu~\text{d}x %\int_\Gamma \tau \cdot %n~u~\text{d}s\\
%  %\int_\Omega  \nabla \cdot \sigma v \text{d}x &= - \int_\Omega f %\nu~\text{d}x
%\end{aligned}
%\end{equation*}
for all $\tau \in \Sigma$, $\nu \in V$, and $u \vert_{\partial \Omega} = 0$.  We assume Neumann boundary conditions so that the integral over $\Gamma$ is $0$. %\lm{I had a go at fixing this, but the formulation is still imprecise. What are the boundary data, what are $\Sigma$ and $V$?}
...
%<<<<<<< wence/edits
We solve an $L^2$ projection problem to give a baseline for the elements. Given either the unit square or unit cube as our domain $\Omega$, we compute the projection of $f$ into the function space $V$ by using a discretization of
\begin{equation*}
  \int u v  \text{ d}x = \int f v \text{ d}x
\end{equation*}
for all $v \in V$.   Recall that the trimmed serendipity elements are denoted with $\mathcal{S}_r^-$ and the tensor product elements with $\mathcal{Q}^-_r$.  In Firedrake, the trimmed serendipity elements use the label \texttt{SminusCurl} or \texttt{SminusDiv} for 1-forms in 2D depending upon the orientation of the edge DOFs, and in 3D, they represent the 1- and 2-forms respectively.  The tensor product elements use the labels \texttt{RTCE} (or \texttt{RTCF}) and  \texttt{NCE} for 1-forms in 2D and 3D, while the 2-forms in 3D are  \texttt{NCF}.  To solve the projection problem in Firedrake, we use a continuous galerkin scheme where we give a bijacobi preconditioner.  
...
\begin{figure}[htbp]
  ...
  \end{subfigure}
  \caption{Results of solving projection problem using 2D and 3D $S^-$ curl elements and tensor product curl elements.  A trendline being lower implies that for a similar mesh size, the error value is lower.  The error in these plots is calculated as the $L^2$ error between a projection of the solution into the finite element space and the exact function evaluated at the DOFs. \label{Projections}} 
\end{figure}

The goal of the projection problem was to establish that the elements behave in the way that we expect them to.  To do this, we plotted the results obtained by using trimmed serendipity elements and tensor product elements for both DOFs vs $L^2$ error and $h$ vs $L^2$ error, seen in \cref{fig:Projection}.  

The expectation is that $\mathcal{S}^-_r$ and $\mathcal{Q}^-_r$ converge at the same rate, illustrated in the plots by parallel trendlines.  These parallel trendlines can be seen in each of the projection plots.  For $r=2$, we see the trendline for $\mathcal{S}^-_r$ below the trendline for $\mathcal{Q}^-_r$, illustrating that that the trimmed serendipity elements achieve a better accuracy for the amount of DOFs required than the tensor product elements.  

In the case of $r=2,3$ the trendlines for $\mathcal{S}^-_r$ are above the trendlines for $\mathcal{Q}^-_r$.  However, considering the elements $\mathcal{Q}^-_3$ and $\mathcal{S}^-_4$, we see that $\mathcal{Q}^-_3$ requires an approximately equal number of DOFs as $\mathcal{S}^-_4$ at the same mesh refinement.  Meanwhile the $\mathcal{S}^-_4$ element yields an extra magnitude of accuracy.
...
%<<<<<<< wence/edits
\end{align*}
where $f(x,y) = 2\pi^2\text{sin}(\pi x)\text{sin}(\pi y) $, yielding the solution $u(x,y) = \text{sin}(\pi x)\text{sin}(\pi y)$. In 3D, we can extend this to $f(x,y,z) = 3\pi^2\text{sin}(\pi x)\text{sin}(\pi y)\text{sin}(\pi z)$ with the solution being $u(x,y,z) = \text{sin}(\pi x)\text{sin}(\pi y)\text{sin}(\pi z)$.  Converting the primal Poisson problem to a weak formulation and discretizing the follow equation, we compute $u \in V$:
\begin{equation*}
    \int_\Omega \nabla u \cdot \nabla v \text{ d}x = \int_\Omega f v \text{ d}x
\end{equation*}
such that the weak form equation holds for all $v \in V$ in the test function space.

The primal Poisson problem makes use of $H^1$ elements, using \texttt{S} for $\mathcal{S}^-_r \Lambda^0$ and \texttt{Lagrange} for $\mathcal{Q}^-_r \Lambda^0$. Then to test \hdiv elements, we compute a solution to a discretization of the mixed Poisson problem:
\begin{align}
     \sigma - \nabla u &= 0 \notag \\
     \nabla \cdot \sigma &= -f \notag \\
     u\vert_{\partial \Omega} &= 0 \notag
\end{align}
In a similar fashion as for the primal Poisson problem, we can create the weak formulation of the mixed Poisson problem.  This results in solving the following weak form equation for $\sigma \in \Sigma$ and $u \in V$
%<<<<<<< wence/edits
...
For both the primal Poisson and mixed Poisson problems, we use an LU preconditioner and ask MUMPS to perform a sparse LU.  For the exact set of solver parameters used, see the test codes.
%=======
...
%<<<<<<< wence/edits
The last numerical experiment that we give here is the cavity resonator problem, making use of the \hcurl elements in 3D.  The time-harmonic Maxwell equations are applied to a domain $\Omega = [0,1]^3$ with perfectly conducting boundary conditions, yielding an eigenvalue problem where $\lambda$ represents a quantity proportional to the frequency squared of the time-harmonic electric field (i.e.\ eigenvalues) and $E$ represents the electric field (i.e.\ eigenfunctions):
\begin{align*}
...
%<<<<<<< wence/edits
\noindent  Based off earlier eigenvalue works \cite{boffi2010finite}, we expect that the rate of convergence be double the order of the finite element used to solve the problem.  This is reflected in the table well for both $\mathcal{S}^-$ and $\mathcal{Q}^-$ elements.  Any location that has a $-$ spot is to be interpreted as the eigenvalue solver did not find that specific eigenvalue in the number of iterations it required to find the first 15 requested eigenvalue-eigenvector pairs.
%=======
...
Since both elements attain the expected convergence rate, we focus on the rest of the results in the table.  Investigating the error in the eigenvalues in the chart compared to the exact values, we see that tensor product elements are able to get results that are up to a magnitude better near the target eigenvalue.  On the other hand, this loss of accuracy from using trimmed serendipity elements results in a reduction in required time to solve for the requested eigenvalues.  At every mesh refinement level, trimmed serendipity elements have nearly half the DOFs of tensor product elements, and correspondingly, require approximately half the time per iteration to solve for the eigenvalues (outside of the case $N=4$).  At higher orders, we expect that this will be even more exaggerated.  
...
%Instead of refining the mesh, we chose to increase the order of the elements used from order$r=2$ to $r=5$ while holding the mesh steady at a size of $16 \times 16 \times 16$.   choose the timing data based on the largest time required for any of the multiplicities that are found. 
...
Trimmed serendipity elements give a new method to use for computing the solution to a discretized PDE on a mesh of squares or cubes. The convergence studies done in each of the numerical experiments show that the trimmed serendipity elements can hit the theoretical rates of convergence that they were predicted to achieve.  While we only illustrate orders 2, 3, and 4 in 2D and 3D, the code for using trimmed serendipity elements in Firedrake is set up to work in both 2D and 3D for arbitrary orders $r$.  

In comparison to tensor product elements $\mathcal{Q}^-_r$, the trimmed serendipity elements $\mathcal{S}^-_r$ make a trade:  lower accuracy in return for less computation, both in terms of DOFs and time required.  In the primal Poisson case and eigenvalue problem case, this trade-off was in favor of the trimmed serendipity elements, while in the mixed Poisson case, we saw the results favoring the tensor product elements. 

Rather than comparing off of order, it can also be beneficial to compare the two elements based off the DOFs that they require.  Consider the 3D mixed Poisson problem while focusing on DOFs vs Error given in \cref{fig:3dMixedDofsError}.  The tensor product elements $\mathcal{Q}^-_2$ required a similar number of DOFs as the trimmed serendipity elements $\mathcal{S}^-_3$.  The return on this investment however was that trimmed serendipity elements gave an extra magnitude of accuracy vs the tensor product element.  Furthermore, in \cref{fig:3dMixedTimeError} the time required for $\mathcal{S}^-_3$ and $\mathcal{Q}^-_2$ was also approximately equal.   Thus while it is helpful to compare $\mathcal{Q}^-_r$ and $\mathcal{S}^-_r$ to see that the trimmed serendipity elements have the proper convergence behavior, the computational comparison that is relevant from a user standpoint is between $\mathcal{Q}^-_r$ and $\mathcal{S}_{r+1}^-$.  

The eigenvalue problem yields another example of comparing the tensor product and trimmed serendipity elements, where instead of refining the mesh, we had refined the order of the element used.  Just as in the mixed Poisson problem, we again see that \cref{fig:EigenvalueDofsError} shows $\mathcal{S}^-_2$ has a higher error for $\lambda = 3$ than $\mathcal{Q}^-_2$.  However comparing against where the DOFs are approximately equal leads to a comparison between $\mathcal{S}^-_3$ and $\mathcal{Q}^-_2$.  In this scenario, we saw that $\mathcal{S}^-_3$ gave almost 2 orders of accuracy better than the tensor product elements did for both eigenvalues $\lambda = 3, 5$.  In this case, we do note that the time required for the third order trimmed serendipity elements did get a little larger than the time required for the second order tensor product elements.
