...
The experiment was done by using Firedrake to create the mass and stiffness matrices as petsc4py objects, then using slepc4py to do the eigenvalue analysis.  The eigenvalue analysis was done by computing an inverted shift to a target of $3.0$, then solving for $15$ eigenvalue-eigenvector pairs.  The SLEPc solve was done using the default (Krylov-Schur) solver with a tolerance level of $1\text{e}-7$, and a couple of specific mumps parameters ({\fontfamily{qcr}\selectfont icntl 14} set to {\fontfamily{qcr}\selectfont 200} and {\fontfamily{qcr}\selectfont icntl 13} set to {\fontfamily{qcr}\selectfont 1}).
