...
An example of writing a discretized PDE and solving it in Firedrake can be found in \cref{lst:mixedPoissonCode}, where we illustrate how to solve the mixed Poisson problem. \akg{We never solve the mixed form of the PDE, we only compute approximate solutions by discretizing it}.  To do this, we first write the mixed Poisson equation on the domain $\Omega := [0, 1] \times [0,1]$ with boundary $\Gamma$ in the weak formulation, where we are solving for $\sigma \in \Sigma$ and $u \in V$, where $\Sigma$ and $V$ are appropriate finite element spaces \akg{In the mixed form, $\Sigma$ and $V$ are just functional spaces - namely \hdiv and $L^2$, not finite element spaces}, such that the following equations are satisfied:
\begin{equation*}
\begin{aligned}
...
\begin{equation}\label{eq:HcurlRtwo}
   ...
   \end{equation}
where these require using two sets of functions, one set on the edges of the reference element to satisfy the functions in the $E_i\Lambda^1$ sum, and one set on the faces of the reference element for the $\tilde{F}_2\Lambda^1$ functions. 

When we then code these sets in Firedrake, we follow a few steps.  The first is to assign the proper DOFs to a reference element.  In order to do this, we have to count the DOFs for each mesh entity on the reference element (vertex, edge, face, and interior).  For example, $\mathcal{S}_2^- \Lambda^1(\square_3)$ should have no DOFs at the vertices (these only come into play for the $H^1$-conforming elements), two DOFs on each edge two DOFs on each face, and no DOFs on the interior.  This lines up with \cref{eq:HcurlRtwo}, where $E_i\Lambda^1$ supplies one DOF to each edge for each $i=0, 1$, and then $\tilde{F}_2\Lambda^1$ supplies two DOFs to each face.  An illustration of this can be seen in the second column, second row of \cref{tab:tsfamily}.

With the number of DOFs assigned to each mesh entity in the reference element we can then go about building the basis functions in an ordered fashion, so that when we assign the basis functions, they are assigned to the proper mesh entity.  For example, two of the basis functions that are contained in the sum of the $E_i\Lambda^1$ sets would be $(y+1)(z+1)dx$ and $x(y+1)(z+1)dx$.  Notice that these functions have no $dy$ or $dz$ portions.  Therefore, we associate these functions to an edge such that the $y$ and $z$ values are constant, forcing $dy = dz = 0$.  If we choose either $y = -1$ or $z = -1$, then the two basis functions we're working with both become zero.  Therefore, we  require that both $y = z = 1$ for the edge that we are working with (seen as the blue edge with 2 red dots on \cref{fig:ReferenceCube}).
...
\draw (0,1,0) -- (0,1,1);
\draw (0,1,1) -- (1,1,1);
\draw[cyan,thick] (1,1,0) -- (1,1,1);
\draw (0,0,1) -- (0,1,1);
\draw (0,0,1) -- (1,0,1);
\draw (1,0,0) -- (1,0,1);
\draw (1,0,1) -- (1,1,1);
\draw (1,0,0) -- (1,1,0);
\filldraw [red] (1,1,0.33) circle (1pt);
\filldraw [red] (1,1,0.66) circle (1pt);
\filldraw [red] (0.66,0.5,1) circle (1pt);
\filldraw [red] (0.33,0.5,1) circle (1pt);

\end{tikzpicture}
\caption{A reference cube where the dotted edge has the properties of $y=1$ and $z=1$ along the entire edge.  The red dots on the blue face and edge represent the DOFs on each of those two mesh entities.  \label{fig:ReferenceCube}}
\end{figure}
...
To get our functions in a FIAT-friendly form, we translate $(y+1)(z+1)dx$ and $x(y+1)(z+1)dx$ to vector notation $((y+1)(z+1), 0, 0)$ $(x(y+1)(z+1),0,0)$.  In this form, we then translate the values $(y+1)$, $(z+1)$ and $x$ in different ways.  Notice that $(y+1)$ and $(z+1)$ on the edge where $y = z = 1$ are constant, whereas $x$ will vary.  The overall conversion to FIAT notation ends with 
\texttt{tuple([(leg(j,x\_mid)*dz[1]*dy[1],0,0)])}
, where \texttt{leg(j, x\_mid)} is replicating the $1$ or $x$ coefficient, and the \texttt{dy[1]} and \texttt{dz[1]} are replicating the values of $(y+1)$ and $(z+1)$ respectively.  After repeating this process for each of the edges and associated basis functions, we then also do a similar process for the face functions in the set $\tilde{F}_2 \Lambda^1$.  
...
\begin{tabular}{llll}
  ...
  \bottomrule
\end{tabular}
\caption{A translation for going from FEEC to Firedrake usage names for tensor product and trimmed serendipity elements.  In 2D, the $0$-forms are the $H^1$ conforming spaces, the $1$-forms are the \hcurl and \hdiv conforming spaces (dependent upon oreintation of the DOFs), and the $2$-forms are the $L^2$ conforming spaces.  For 3D, the $0$-forms are the $H^1$ conforming spaces, the $1$-forms are \hcurl spaces, $2$-forms are \hdiv spaces, and $3$-forms are $L^2$ spaces. \label{tab:FiredrakeNames}}
