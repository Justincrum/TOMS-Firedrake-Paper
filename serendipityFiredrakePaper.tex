...
\justin{Firedrake is a Python package for computing the solution to PDEs by using the finite element method.  By leveraging the Unified Form Language \cite{Logg:2012,alnaes2014unified}, it is able to provide a flexible and high level interface for the solution of PDEs that is reminiscent of their mathematical formulation.  Rob -- How does this look?}{\rck{Good}}
...
We assume homogeneous Dirichlet boundary conditions so that the integral over $\Gamma$ is $0$. %\lm{I had a go at fixing this, but the formulation is still imprecise. What are the boundary data, what are $\Sigma$ and $V$?}
...
\end{figure}
   ...
   \justin{Finally, while reducing the number of DOFs will impact the accuracy of the element, we are interested in determining if there is a way to make this choice to gain accuracy while not increasing the DOFs required to do so.  
   For example, in \cref{fig:DegsDofs}, the trimmed serendipity elements at order $3$ require a similar number of DOFs as the tensor product element at order $2$.  
   Thus we believe that a  user making a choice to use trimmed serendipity elements at an increased order may be a meaningful way to achieve a more accurate solution.} %\lm{I removed the references to $r, k, n =$ since that notation is not yet introduced and cluttered this paragraph}
