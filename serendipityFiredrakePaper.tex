...
With the number of DOFs assigned to each mesh entity in the reference element we can then go about building the basis functions in an ordered fashion, so that when we assign the basis functions, they are assigned to the proper mesh entity.  Note that \cref{eq:HcurlRtwo} doesn't explicitly give a way to order the basis functions.  Instead, we need to use the properties of the basis functions to determine which mesh entities they should be associated with. For example, two of the basis functions that are contained in the sum of the $E_i\Lambda^1$ sets would be $(y+1)(z+1)dx$ and $x(y+1)(z+1)dx$.  Notice that these functions have no $dy$ or $dz$ portions.  Therefore, we associate these functions to an edge such that the $y$ and $z$ values are constant, forcing $dy = dz = 0$.  If we choose either $y = -1$ or $z = -1$, then the two basis functions we're working with both become zero.  Therefore, we  require that both $y = 1$ and $z = 1$ for the edge that we are working with (seen as the blue edge labeled with an "e" that has 2 red dots on \cref{fig:ReferenceCube}).
...
\draw (1,0,1) -- (1,1,1);
\draw (1,0,0) -- (1,1,0);
\draw[thick, ->] (2,1,0.5) -- (1,1,0.5);
\draw (2.1,1,0.5) node {e};
\draw[thick, ->] (-1,0.4,1) -- (0.5,0.4,1);
\draw (-1.1,0.4,1) node {F};
...
\noindent For this case, we would have one of the basis functions being $y^jz^k(x+1)dydz$.  The $dydz$ represents a $2$-form, and similar to before, we know that we do not want this function to be identically zero on the face we associate it with.  Thus, out of the six faces, we know that either face where $y$ or $z$ is constant will result in $dy = 0$ or $dz = 0$.  Furthermore, if we choose the face where $x = -1$, then the function is identially zero.  Hence, we associate this function with the face were $x = 1$.  The rest of the process for defining the $2$-form basis functions is similar to the process for the $1$-form basis functions.
