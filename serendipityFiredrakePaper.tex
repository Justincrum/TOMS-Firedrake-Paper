...
%   in section~\ref{sec:buildcap}.
  ...
  \begin{enumerate}
      \item Show how Firedrake creates a software environement that can be used to implement new elements as well as allow the user to call the new elements with only small changes to their existing codes. 
      \item Illustrate trimmed serendipity finite elements hitting the theoretical bounds that are predicted in terms of convergence rates. 
      \item Examine the costs and benefits of using trimmed serendipity elements within the confines of typical toy problems for numerical analysis by comparing them to results obtained using tensor product elements on the same problems.
  \end{enumerate}
