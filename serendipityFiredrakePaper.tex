...
\end{aligned}
\end{equation*}
for all $\tau \in \Sigma$, $\nu \in V$, and $u \vert_{\partial \Omega} = 0$. %\lm{I had a go at fixing this, but the formulation is still imprecise. What are the boundary data, what are $\Sigma$ and $V$?}
...
\begin{equation}
\begin{tabular}{rllll}
$(\sigma,\tau) + (\nabla\cdot\tau, u)$ & $=~~ (\tau\cdot\hat n, u),$ & $\forall~ \tau\in\Sigma$, \\
$(\nabla\cdot\sigma, v)$ & $= ~~-(f,v),$ & $\forall~ v\in V$.
\end{tabular}
\end{equation}
