...
\begin{abstract}
  ...
  In addition to the four elements present on the Periodic Table of Finite Elements, much research has been put into trimmed serendipity finite elements.  Trimmed serendipity elements give another method for solving partial differential equations on cubical meshes using finite elements.  However, as a relatively newer element in comparison to tensor product elements and serendipity elements, the implementation of trimmed serendipity elements has fallen behind.  We present an implementation in Firedrake for $H($curl$)$ and $H($div$)$ conforming trimmed serendipity elements in both $n=2,3$ dimensions for arbitrary orders $r$.  
