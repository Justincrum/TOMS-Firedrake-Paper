...
{\textsc{Justin Crum$^1$, Cyrus Cheng$^2$, Joshua A. Levine$^1$, Robert Kirby$^3$, David Ham$^2$, Lawrence Mitchell$^4$, Andrew Gillette$^1$\\[1mm]}
\small{\text{}\qquad $^1$ University of Arizona,\quad $^2$ Imperial College London,\quad $^3$ Baylor University,\quad $^4$Durham University} } % Author names and institution
...
\item Confirm convergence rates.
...
inside of a FIAT element.  When a Firedrake code is asked to create a function space via
\begin{center}
{\fontfamily{qcr}\selectfont FunctionSpace(mesh, "SminusCurl", r)} \\
\end{center}
it finds the appropriate set of basis functions, based on the portions of Equation~(\ref{eq:HCurlTrimmedSerendipityBasis}) required for the chosen degree (i.e.\ \texttt{r}).
...
\begin{enumerate}\compresslist
    \item Apply trimmed serendipity \\ elements in specific application contexts. 
    \item Implement regular serendipity elements for further comparison.
...
Work supported in part by NSF \\ Collaborative Research Awards\\ DMS-1913094 and DMS-1912653.
\begin{center}
